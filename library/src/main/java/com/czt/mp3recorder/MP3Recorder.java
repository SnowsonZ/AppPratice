...
public class MP3Recorder {
     ...
     * 16 bits pcm
     */
    public MP3Recorder(String dir, String fileName) {
        File parentFile = new File(dir);
        if (!parentFile.exists()) {
            parentFile.mkdirs();
        }
        mRecordFile = new File(parentFile, fileName);
    ...
    public void start() throws IOException {
        //再次录音时,删除上次录音文件
        if (status == AudioStatus.STATUS_STOP) {
            if (mRecordFile.exists()) {
                mRecordFile.delete();
                initLame();
            }
    ...
    public boolean isRecording() {
        return status == AudioStatus.STATUS_PAUSE || status == AudioStatus.STATUS_START;
    }
